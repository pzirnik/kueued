...
QList< IR_item > Database::get_IR_status( const QStringList& srs, const QString& dbname )
{
    QSqlDatabase db;
    db = QSqlDatabase::database( dbname );
    db.transaction();
    QSqlQuery query( db );
    QList< IR_item > list;
    QString srlist = srs.join(",");
    
    QString q(  "select "
                " sr.SR_NUM as ID, "
                " activity.EVT_STAT_CD as STATUS "
                "from "
                " (select SR_NUM,ROW_ID from SIEBEL.S_SRV_REQ where SR_NUM in (" + srlist + ")) sr, "
                " SIEBEL.S_EVT_ACT activity "
                "where "
                " activity.SRA_SR_ID = sr.row_id and "
                " activity.TODO_CD='SLA Initial Response' AND "
                " activity.PUBLISH_CD='Internal' AND "
                " activity.CAL_TYPE_CD='Calendar and Activities' ");
    ...
    Debug::logQuery( query, db.connectionName() );
    Debug::print("IR_item", " srlist = " + srlist);
    
    while ( query.next() )
    {
        IR_item ir;
        
        ir.sr = query.value( 0 ).toString();
        ir.status = query.value( 1 ).toString();
        Debug::print("get_IR (sr) ", ir.sr);
        Debug::print("get_IR (status) ", ir.status);
        list.append( ir );
    }
...
QList< SiebelItem > Database::getQmonSrs( const QString& dbname, const QString& mysqlname, const QString& reportname )
{
    ...
    query.prepare(  "SELECT sr.SR_NUM  ID, "
                    " queue.LOGIN              QUEUE, "
                    "  CASE "
                    "   WHEN sr.BU_ID = '0-R9NH' THEN 'Default Organization' "
                    "   WHEN sr.BU_ID = '1-AHT'  THEN 'EMEA' "
                    ...
                    "  END                     GEO, "
                    " cal.NAME                HOURS, "
                    " sr.SR_SUB_STAT_ID      STATUS, "
                    " sr.SR_SEV_CD           SEVERITY, "
                    " sr.SR_SUBTYPE_CD       SOURCE, "
                    " sr.X_RESPOND_VIA       RESPOND_VIA, "
                    " sr.CREATED, "
                    " sr.LAST_UPD            LAST_UPDATE, "
                    " NVL(sr.EXP_CLOSE_DT,TO_DATE('01-01-1970','MM-DD-YYYY'))    SLA, "
                    " sr.X_SUPPORT_PROG      SUPPORT_PROGRAM, "
                    " e.NAME                SUPPORT_PROGRAM_LONG, "
                    " prd.NAME                ROUTING_PRODUCT, "
                    " sr.X_SUPP_GRP_ROUTING  SUPPORT_GROUP_ROUTING, "
                    " sr.SR_TYPE_CD          INT_TYPE, "
                    " sr.X_SR_SUB_TYPE       SUBTYPE, "
                    " e.ENTL_PRIORITY_NUM   SERVICE_LEVEL, "
                    " SR_TITLE                BRIEF_DESC, "
                    " flag.ATTRIB_11           CRITSIT, "
                    " flag.ATTRIB_56           HIGH_VALUE, "
                    " ext.NAME                CUSTOMER, "
                    " REGEXP_REPLACE(g.WORK_PH_NUM,'(.*)'||CHR(10)||'(.*)','\\1')     CONTACT_PHONE, "
                    " REGEXP_REPLACE(g.WORK_PH_NUM,'(.*)'||CHR(10)||'(.*)','\\2')     FORMAT_STRING, "
                    " g.FST_NAME            CONTACT_FIRSTNAME, "
                    " g.LAST_NAME           CONTACT_LASTNAME, "
                    " g.EMAIL_ADDR          CONTACT_EMAIL, "
                    " g.JOB_TITLE           CONTACT_TITLE, "
                    " g.PREF_LANG_ID        CONTACT_LANG, "
                    " REGEXP_REPLACE(sr.X_ONSITE_PH_NUM,'(.*)'||CHR(10)||'(.*)','\\1') ONSITE_PHONE, "
                    " REGEXP_REPLACE(sr.X_ONSITE_PH_NUM,'(.*)'||CHR(10)||'(.*)','\\2') ONSITE_FORMAT_STRING, "
                    " sr.DESC_TEXT           DETAILED_DESC, "
                    " sr.SR_CATEGORY_CD      CATEGORY, "
                    " sr.ROW_ID, "
                    " sr.X_ALT_CONTACT, "
                    " sr.X_DEFECT_NUM, "
                    " ext.X_ORACLE_CUSTOMER_ID, "
                    ...
                    "from "
                    "  siebel.s_srv_req sr "
                    "  LEFT OUTER JOIN siebel.s_user queue ON sr.owner_emp_id = queue.row_id, "
                    "  siebel.s_srv_req_x srx "
                    "  LEFT OUTER JOIN siebel.s_user subowner ON srx.attrib_07 = subowner.row_id, "
                    "  siebel.s_user u, "
                    "  siebel.s_contact c, "
...
/*                    "UNION "
                    ...
                    " flag.ATTRIB_56           HIGH_VALUE, "
                    " o.NAME                CUSTOMER, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_PHONE, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  FORMAT_STRING, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_FIRSTNAME, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_LASTNAME, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_EMAIL, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_TITLE, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  CONTACT_LANG, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  ONSITE_PHONE, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  ONSITE_FORMAT_STRING, "
                    " DECODE(sr.OWNER_EMP_ID,'0-1','1',NULL,'0')  DETAILED_DESC, "
                    " sr.SR_CATEGORY_CD      CATEGORY, "
                    " sr.ROW_ID, "
                    " sr.X_ALT_CONTACT, "
                    " sr.X_DEFECT_NUM, "
